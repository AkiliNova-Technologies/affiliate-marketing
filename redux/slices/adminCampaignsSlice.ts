...
export const fetchMarketerCampaigns = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      return {
        campaigns: (data.data || data.campaigns || data) as MarketerCampaignDetail[],
        total: data.total || data.count || 0,
      };
    } catch (err) {
      return rejectWithValue(handleApiError(err));
...
const adminCampaignsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchAdminCampaigns.pending, (state) => { state.loading = true; state.error = null; })
      ...
      .addCase(fetchMarketerCampaigns.pending, (state) => { state.marketerLoading = true; state.error = null; })
...
export const { clearMarketerCampaigns, clearError } = adminCampaignsSlice.actions;
export default adminCampaignsSlice.reducer;

export const selectAdminCampaigns = (s: { adminCampaigns: AdminCampaignsState }) => s.adminCampaigns.campaigns;
export const selectAdminCampaignsTotal = (s: { adminCampaigns: AdminCampaignsState }) => s.adminCampaigns.total;
export const selectAdminCampaignsLoading = (s: { adminCampaigns: AdminCampaignsState }) => s.adminCampaigns.loading;
export const selectMarketerCampaigns = (s: { adminCampaigns: AdminCampaignsState }) => s.adminCampaigns.marketerCampaigns;
export const selectMarketerCampaignsTotal = (s: { adminCampaigns: AdminCampaignsState }) => s.adminCampaigns.marketerTotal;
export const selectMarketerCampaignsLoading = (s: { adminCampaigns: AdminCampaignsState }) => s.adminCampaigns.marketerLoading;
export const selectAdminCampaignsError = (s: { adminCampaigns: AdminCampaignsState }) => s.adminCampaigns.error;